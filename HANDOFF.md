...
2. **OCR accuracy across different real products/label shapes** — confirmed working end-to-end on real hardware for a flat-labeled item (Zeiss lens wipes, `662834503500`), including recovering the correct value from a real, imperfect camera capture and correctly ignoring an unrelated printed code elsewhere on the package. Failed on a Deer Park individual water bottle — not yet root-caused; a curved/cylindrical label warps the digit line more than a flat one does, which is the leading suspect, but the actual OCR debug text from that attempt wasn't captured. Next time this comes up, get the `lastOcrDebug` text (shown on the scan debug readout / recovery flow) for the specific failing product before changing anything.

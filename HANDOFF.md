...
2. **OCR fallback on real hardware** — verified end-to-end only in browser preview (real barcode image with bars blacked out, file-input path). On the actual R1, OCR fallback consistently timed out (~20s, added in `8cda037`) rather than completing or failing cleanly - the Tesseract Worker/WASM pipeline appears to hang there. Added a "Diagnostics" item to the Home menu (`enterDiag()` in `js/app.js`) that isolates plain `Worker` messaging, main-thread `WebAssembly.instantiate`, and WASM-instantiate-inside-a-Worker as three independent tests, to find out which primitive is actually broken before deciding whether OCR is viable on this device at all. This is a developer tool, not a real feature - remove the Home menu entry once the underlying question is resolved.

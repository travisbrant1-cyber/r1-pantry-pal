...
**OCR fallback reads the printed digit line, not the bars.** Added after real-hardware testing kept failing to decode bars even once resolution, capture, and focus-distance issues were all fixed — the working theory is the fixed-focus lens plus this camera's limits leave too few pixels-per-bar-module even at the correct distance, while the much larger printed digits underneath may still be legible. Implementation: `js/vendor/tesseract/` (Tesseract.js + the `eng` fast-trained LSTM model, self-hosted like ZXing to keep the offline-first principle — see below for why this is an exception in practice). Only triggers after the ZXing burst already failed, and only ever loads (~8MB) at that point, not on every scan. Any candidate digit string is required to pass the standard GTIN check-digit algorithm (`isValidGTIN()`) before being accepted as a real barcode — this is the safety net against a misread digit silently pulling up the wrong product, on top of the existing confirm-before-save screen.
**Extraction is scoped per OCR'd line, not the whole recognized text.** First real-hardware attempt concatenated tokens across `result.data.text` as one flattened blob (`extractValidBarcodeFromLineText()` used to be applied to the whole thing) - real packages usually have other printed numbers besides the barcode (lot/item codes), and merging tokens across unrelated lines produced garbage like `828345035005273` instead of the real `662834503500`. Now uses `result.data.lines` (Tesseract reports per-physical-line text with its own bounding box) and only tries concatenating tokens within one line (`extractValidBarcodeFromOcrLines()`) - verified by feeding a two-line synthetic image (an unrelated numeric line plus the real barcode's digit line) and confirming only the correct line's value gets extracted.
...
2. **OCR fallback on real hardware** — verified end-to-end only in browser preview (real barcode image with bars blacked out, file-input path). Real-device debugging history: first timed out (~20s) rather than completing or failing cleanly, which looked like a Worker/WASM hang; the "Diagnostics" Home menu item (`enterDiag()` in `js/app.js`) was added to isolate that, and on real hardware all of plain `Worker` messaging, main-thread `WebAssembly.instantiate`, WASM-inside-a-Worker, and even a full real Tesseract worker init all passed quickly - so it's *not* a platform incompatibility. The remaining suspect (not yet confirmed on real hardware) is that `recognize()` against the camera's full native resolution (e.g. 1080×1920, ~2MP) is just genuinely slow on this device's CPU with a non-SIMD WASM OCR engine, longer than the original timeout allowed. Two changes address that: `downscaleForOcr()` now caps the image at 900px on the long edge before OCR (digits don't need full sensor resolution), and `OCR_TIMEOUT_MS` was raised to 45s; a 5th Diagnostics test (`runDiagOcrRecognizeTest`) times `recognize()` against a synthetic same-size image to get a concrete number. This is a developer tool, not a real feature - remove the Home menu entry once the underlying question is resolved.
